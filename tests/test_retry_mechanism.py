...
def test_add_update(client, base_ddo_url, events_object, monkeypatch):
    ...
    assert queue_element["next_retry"] == int((after_5_minutes_and_a_bit + timedelta(minutes=10)).timestamp())
    ...
    assert queue_element["next_retry"] == int((after_another_10_minutes_and_a_bit + timedelta(minutes=15)).timestamp())
    ...
    queue_element = mechanism.get_by_id(rm_id)
    assert queue_element["number_retries"] == 0
    assert queue_element["next_retry"] == int(after_another_10_minutes_and_a_bit.timestamp())
