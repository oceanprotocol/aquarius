...
def publishDDO(did,flags,data):
    print(f'publishDDO with flags: {flags}')
    transaction = events_contract.functions.newDDO(did,flags,data).buildTransaction()
    transaction.update({ 'gasPrice' : 0 })
    transaction.update({ 'nonce' : web3.eth.getTransactionCount(account1.address) })
    signed_tx = web3.eth.account.signTransaction(transaction, account1.privateKey)
    txn_hash = web3.eth.sendRawTransaction(signed_tx.rawTransaction)
    txn_receipt = web3.eth.waitForTransactionReceipt(txn_hash)
    return(txn_receipt)

def updateDDO(did,flags,data):
    print(f'updateDDO with flags: {flags}')
    transaction = events_contract.functions.updateDDO(did,flags,data).buildTransaction()
    transaction.update({ 'gasPrice' : 0 })
    transaction.update({ 'nonce' : web3.eth.getTransactionCount(account1.address) })
    signed_tx = web3.eth.account.signTransaction(transaction, account1.privateKey)
    txn_hash = web3.eth.sendRawTransaction(signed_tx.rawTransaction)
...
def test_publish_and_update_ddo(client,base_ddo_url):
    ...
    publishDDO(did,bytes([flags]),data)    
    time.sleep(10)
    ...
    ddo_string=json.dumps(ddo_event_sample)
    data = Web3.toBytes(text=ddo_string)
    updateDDO(did,bytes([flags]),data)
    time.sleep(15)
    published_ddo = get_ddo(client,base_ddo_url,did)
    assert published_ddo['id'] == did
