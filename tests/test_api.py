...
from tests.helpers import (get_event, get_web3, new_ddo, send_create_update_tx,
                           test_account1)
...
def test_post_with_no_valid_ddo(client, base_ddo_url, events_object):
    ...
    ddo = new_ddo(
        test_account1, get_web3(), f'dt.{block}', json_dict_no_valid_metadata
    )
    ddo_string = json.dumps(dict(ddo.items()))
    _receipt = send_create_update_tx(
        'create', ddo.id, bytes([1]),
        lzma.compress(Web3.toBytes(text=ddo_string)), test_account1
    )
    get_event(EVENT_METADATA_CREATED, block, ddo.id)
    events_object.process_current_blocks()
    try:
        published_ddo = get_ddo(client, base_ddo_url, ddo.id)
        assert not published_ddo, 'publish should fail, Aquarius validation ' \
...
def test_validate_remote(client_with_no_data, base_ddo_url):
    post = run_request(
        client_with_no_data.post,
        base_ddo_url + '/validate-remote', data={}
    )
