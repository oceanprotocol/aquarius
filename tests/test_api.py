...
def test_create_asset(client):
    ...
    post = client.post(BaseURLs.BASE_PROVIDER_URL + '/assets/metadata/001',
                       data=json.dumps(json_dict),
                       content_type='application/json')
    rv = client.get(
        BaseURLs.BASE_PROVIDER_URL + '/assets/metadata/%s' % '001',
        content_type='application/json')
...
def test_post_with_no_metadata(client):
    post = client.post(BaseURLs.BASE_PROVIDER_URL + '/assets/metadata/%s' % '002',
...
def test_post_with_no_valid_metadata(client):
    post = client.post(BaseURLs.BASE_PROVIDER_URL + '/assets/metadata/%s' % '003',
...
def test_update_asset_metadata(client):
    post = client.post(BaseURLs.BASE_PROVIDER_URL + '/assets/metadata/%s' % '004',
                       data=json.dumps(json_before),
                       content_type='application/json')
    put = client.put(BaseURLs.BASE_PROVIDER_URL + '/assets/metadata/%s' % '004',
               data=json.dumps(json_update),
               content_type='application/json')
    rv = client.get(
        BaseURLs.BASE_PROVIDER_URL + '/assets/metadata/%s' % '004',
        content_type='application/json')
