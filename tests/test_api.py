...
def test_create_asset(client):
    ...
    post = client.post(BaseURLs.BASE_AQUARIUS_URL + '/assets/metadata',
                       data=json.dumps(json_dict),
                       content_type='application/json')
    rv = client.get(
        BaseURLs.BASE_AQUARIUS_URL + '/assets/metadata/%s' % json.loads(post.data.decode('utf-8'))['assetId'],
        content_type='application/json')
    assert json_dict['base']['name'] in json.loads(rv.data.decode('utf-8'))['base']['name']
    client.delete(BaseURLs.BASE_AQUARIUS_URL + '/assets/metadata/%s' % json.loads(post.data.decode('utf-8'))['assetId'])
...
def test_query_metadata(client):
    post = client.post(BaseURLs.BASE_AQUARIUS_URL + '/assets/metadata',
                       data=json.dumps(json_before),
                       content_type='application/json')
    post2 = client.post(BaseURLs.BASE_AQUARIUS_URL + '/assets/metadata',
                        data=json.dumps(json_dict),
                        content_type='application/json')
    assert len(json.loads(client.post(BaseURLs.BASE_AQUARIUS_URL + '/assets/metadata/query',
    ...
    client.delete(BaseURLs.BASE_AQUARIUS_URL + '/assets/metadata/%s' % json.loads(post.data.decode('utf-8'))['assetId'])
    client.delete(
        BaseURLs.BASE_AQUARIUS_URL + '/assets/metadata/%s' % json.loads(post2.data.decode('utf-8'))['assetId'])
