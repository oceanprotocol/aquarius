...
class Events:
    ...
    def __init__(self, rpc, contract_address,config_file):
        ...
        if not self._web3.isAddress(contract_address):
            logger.error(f"Contract address {contract_address} is not a valid address. Events thread not starting")
            self._contract=None
            return
        path = './aquarius/artifacts/DDO.json'
        data=json.load(open(path))
        self._contract=self._web3.eth.contract(address=contract_address, abi=data['abi'])
    ...
    def start_events_monitor(self):
        ...
        if self._contract is None:
            ...
            return
        logger.error(f'Starting the events monitor on contract {self._contract_address}.')
        t = Thread(
    ...
    def run_monitor(self):
        while True:
            try:
                ...
                events_filter = self._contract.events.updateDDOEvent.createFilter(fromBlock=last_block)
    ...
    def processNewDDO(self,event):
        ...
        for service in _record['service']:
            service['attributes']['main']['dateCreated'] = format_timestamp(data['created'])
        ...
        if not is_valid_dict_remote(get_metadata_from_services(_record['service'])['attributes']):
            errors = list_errors(list_errors_dict_remote,get_metadata_from_services(_record['service'])['attributes'])
            logger.error(errors)
            return
        try:
    ...
    def decode_ddo(self,rawddo,flags):
        ...
        try:
            ...
            logger.error(f'encounterd an error while decoding the ddo: {str(err)}')
            return None
